...
vp_named AS (
  SELECT
    ...
    vp.price
  FROM vp
  INNER JOIN vendor  AS v ON v.vendor_id  = vp.vendor_id
  INNER JOIN product AS p ON p.product_id = vp.product_id
),
customer_count AS (
...
SELECT
  vn.vendor_name,
  vn.product_name,
  ROUND(vn.price * 5 * cc.num_customers, 2) AS total_revenue
FROM vp_named AS vn
...
Secoesce null valnd, coalues to 0 (if you don't have null values, figure out how to rearrange your query so you do.) 
...
SET current_quantity = COALESCE(
  (
    ...
    ORDER BY DATE(vi.market_date) DESC
    LIMIT 1
  ),
  0
);
